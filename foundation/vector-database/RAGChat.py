...
embeddings = GoogleGenerativeAIEmbeddings(
    ...
    output_dimensionality=os.environ["OUTPUT_DIMENSIONALITY"]
)
...
def ask_agent(query):
    ...
    retriever = vector_store.as_retriever(search_kwargs={"k": os.environ["K"]})

    # print(f"retriever {retriever}")
    # Define the Prompt Template
    template = """
    You are a helpful assistant. Use the following pieces of retrieved context 
    from image text to answer the question. 
    ...
    # Build the RAG Chain
    # The Chain: Query -> Retrieve -> Prompt -> LLM
    rag_chain = (
    ...
    if "I don't know" in response or "I'm sorry" in response:
        ask_question(vector_store, query)
